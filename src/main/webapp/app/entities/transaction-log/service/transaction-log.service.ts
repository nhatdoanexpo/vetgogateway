...
type RestOf<T extends ITransactionLog | NewTransactionLog> = Omit<T, 'createdDate'> & {
...
export class TransactionLogService {
  ...
  addTransactionLogToCollectionIfMissing<Type extends Pick<ITransactionLog, 'id'>>(
    ...
    if (transactionLogs.length > 0) {
      const transactionLogCollectionIdentifiers = transactionLogCollection.map(
        transactionLogItem => this.getTransactionLogIdentifier(transactionLogItem)!
      );
  ...
  protected convertDateFromClient<T extends ITransactionLog | NewTransactionLog | PartialUpdateTransactionLog>(
    ...
    return {
      ...transactionLog,
      createdDate: transactionLog.createdDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTransactionLog: RestTransactionLog): ITransactionLog {
    return {
      ...restTransactionLog,
      createdDate: restTransactionLog.createdDate ? dayjs(restTransactionLog.createdDate) : undefined,
    };
  }
